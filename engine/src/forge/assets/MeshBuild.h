...
// is > 0 — ends already at r = 0 are sealed by the pole collapse. Triangles
// wind counter-clockwise seen from outside (#116: winding is what
// RecomputeNormalsWelded rebuilds normals from after any edit).
// Returns false and leaves `out` empty for degenerate input: fewer than two
// distinct points, every radius zero, a negative radius, non-finite values.
...
// Returns false for degenerate input: fewer than three distinct section
// points, zero section area, fewer than two distinct path points, non-finite
// values.
